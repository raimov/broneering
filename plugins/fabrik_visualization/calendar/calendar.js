...
var fabrikCalendar = new Class({
	...
	_makeEventRelDiv: function (entry, opts, aDate, target)
	{
		...
		var kogus = entry.kogus;
			var test = (opts.width.slice(0, -2));
			var test2 = test / kogus;
		...
		var style = {
				...
				'width': test2 + 'px',
				'cursor': 'pointer',
				'margin-left': opts['margin-left'] / kogus,
		...
		if (this.options.j3) {
			...
			dataContent = Joomla.JText._('PLG_VISUALIZATION_CALENDAR_EVENT_START_END').substitute(replace) + 'aaaaaaaaaa';
